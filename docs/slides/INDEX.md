...
- **`../../slides.html`** — the workshop browser deck on the Argonne template
  ...
  [`../../scripts/build_html.py`](../../scripts/build_html.py) from
  `../../archive/slides-classic.html`.
- **`../../archive/slides-classic.html`** — the original serif-themed deck; kept as
  the HTML build source (slide markup + nav JS).
...
Build everything with `make` (see the repo `Makefile`): `make html`, `make pptx`, or `make slides`.
