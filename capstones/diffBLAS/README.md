...
The problem is that `DGEMM_D` may be called from contexts where a proper subset of `alpha`, `a`, `b`, `beta` are not active and therefore the provided `DGEMM_D` is not appropriate for either correctness and/or performance reasons.
...
Design a tool that parses the code (fror `DGEMM_D`) into an intermediate representation and uses the provided activity descriptor to manage declarations and prune statements in the code. This approach assumes compiler infrastructure.
...
4. Explore options to parse and manipulate the fortran source
5. Create a transformation pass that accepts the activity pattern and appropriately modified the code.
