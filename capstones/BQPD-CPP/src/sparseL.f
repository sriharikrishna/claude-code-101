...
c  Current version dated 22/11/00
...
c  set up pointers for row-wise sparse structure
      ...
      if(p(n)+q(n).gt.mxws)then
        ifail=7
        write(nout,*)'BQPD: increase mxws by ',p(n)+q(n)-mxws
        return
      endif
      q(n)=p(n)-1
      i=nu+1
...
c     write(nout,*)'irow,lastr,mxws =',irow,lastr,mxws
      ...
      if(lastr.eq.0)then
        if(mxws.lt.len)then
          ...
          write(nout,*)'BQPD: increase mxws by ',len-mxws
          ifail=7
        else
          irow=row
          q(row)=0
...
c  compress ws
    ...
    3 continue
      ...
      if(mxws.lt.qrow+len_)then
        ...
        write(nout,*)'BQPD: increase mxws by ',qrow+len-mxws
        ifail=7
        return
      endif
