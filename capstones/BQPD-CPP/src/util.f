...
cDMG  external  ieee_handler, abort
cDMG  integer   ieee_flags, ieeer, ieee_handler
cDMG  external  ieee_flags
cDMG  character out*16
cDMG  out = ''
cDMG  ieeer = ieee_flags ( 'clearall','all','',out )
cDMG  ieeer=ieee_handler('clear','overflow',abort)
...
c  should be removed
      ...
      enddo
cDMG  xlen=sqrt(xlen)
cDMG  ieeer=ieee_flags ( 'get','exception','',out )
cDMG  if(out.eq.'overflow')then
      if (xlen + xlen .le. xlen) then
         if (xlen .gt. 0) then
            call linf(n,x,xmx,i)
c           xmx=max(xmx,1.D0) %this is needed if normalization is always used
            ...
            enddo
            xlen=xmx*sqrt(xlen)
cDMG    ieeer=ieee_flags ( 'clear','overflow','',out )
         endif
      else
         xlen=sqrt(xlen)
      endif
cDMG  ieeer=ieee_handler('set','overflow',abort)
      return
      end
